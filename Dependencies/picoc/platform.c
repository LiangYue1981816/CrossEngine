...
#define CALL_ENABLE "Enable();"
#define CALL_DISABLE "Disable();"
#define CALL_UPDATE "Update(__deltaTime);"
#define CALL_FIXEDUPDATE "FixedUpdate(__fixedDeltaTime);"

void PicocCallEnable(Picoc *pc)
{
	struct Value *FuncValue = NULL;

	if (!VariableDefined(pc, TableStrRegister(pc, "Enable")))
		ProgramFailNoParser(pc, "Enable() is not defined");

	VariableGet(pc, NULL, TableStrRegister(pc, "Enable"), &FuncValue);

	if (FuncValue->Typ->Base != TypeFunction)
		ProgramFailNoParser(pc, "Enable is not a function - can't call it");

	if (FuncValue->Val->FuncDef.NumParams != 0)
		ProgramFailNoParser(pc, "Enable have not param - can't call it");

	if (FuncValue->Val->FuncDef.ReturnType != &pc->VoidType)
		ProgramFailNoParser(pc, "Enable have not return value - can't call it");

	PicocParse(pc, "startup", CALL_ENABLE, strlen(CALL_ENABLE), TRUE, TRUE, FALSE, TRUE);
}

void PicocCallDisable(Picoc *pc)
{
	struct Value *FuncValue = NULL;

	if (!VariableDefined(pc, TableStrRegister(pc, "Disable")))
		ProgramFailNoParser(pc, "Disable() is not defined");

	VariableGet(pc, NULL, TableStrRegister(pc, "Disable"), &FuncValue);

	if (FuncValue->Typ->Base != TypeFunction)
		ProgramFailNoParser(pc, "Disable is not a function - can't call it");

	if (FuncValue->Val->FuncDef.NumParams != 0)
		ProgramFailNoParser(pc, "Disable have not param - can't call it");

	if (FuncValue->Val->FuncDef.ReturnType != &pc->VoidType)
		ProgramFailNoParser(pc, "Disable have not return value - can't call it");

	PicocParse(pc, "startup", CALL_DISABLE, strlen(CALL_DISABLE), TRUE, TRUE, FALSE, TRUE);
}

void PicocCallUpdate(Picoc *pc, double deltaTime)
{
	struct Value *FuncValue = NULL;

	if (!VariableDefined(pc, TableStrRegister(pc, "Update")))
		ProgramFailNoParser(pc, "Update() is not defined");

	VariableGet(pc, NULL, TableStrRegister(pc, "Update"), &FuncValue);

	if (FuncValue->Typ->Base != TypeFunction)
		ProgramFailNoParser(pc, "Update is not a function - can't call it");

	if (FuncValue->Val->FuncDef.NumParams != 1 || FuncValue->Val->FuncDef.ParamType[0]->Base != TypeFP)
		ProgramFailNoParser(pc, "Update have not valid param - can't call it");

	if (FuncValue->Val->FuncDef.ReturnType != &pc->VoidType)
		ProgramFailNoParser(pc, "Update have not return value - can't call it");

	VariableDefinePlatformVar(pc, NULL, "__deltaTime", &pc->FPType, (union AnyValue *)&deltaTime, FALSE);
	PicocParse(pc, "startup", CALL_UPDATE, strlen(CALL_UPDATE), TRUE, TRUE, FALSE, TRUE);
}

void PicocCallFixedUpdate(Picoc *pc, double fixedDeltaTime)
{
	struct Value *FuncValue = NULL;

	if (!VariableDefined(pc, TableStrRegister(pc, "FixedUpdate")))
		ProgramFailNoParser(pc, "FixedUpdate() is not defined");

	VariableGet(pc, NULL, TableStrRegister(pc, "FixedUpdate"), &FuncValue);

	if (FuncValue->Typ->Base != TypeFunction)
		ProgramFailNoParser(pc, "FixedUpdate is not a function - can't call it");

	if (FuncValue->Val->FuncDef.NumParams != 1 || FuncValue->Val->FuncDef.ParamType[0]->Base != TypeFP)
		ProgramFailNoParser(pc, "FixedUpdate have not valid param - can't call it");

	if (FuncValue->Val->FuncDef.ReturnType != &pc->VoidType)
		ProgramFailNoParser(pc, "FixedUpdate have not return value - can't call it");

	VariableDefinePlatformVar(pc, NULL, "__fixedDeltaTime", &pc->FPType, (union AnyValue *)&fixedDeltaTime, FALSE);
	PicocParse(pc, "startup", CALL_FIXEDUPDATE, strlen(CALL_FIXEDUPDATE), TRUE, TRUE, FALSE, TRUE);
}
