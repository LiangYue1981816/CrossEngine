...
int TableSet(Picoc *pc, struct Table *Tbl, char *Key, struct Value *Val, const char *DeclFileName, int DeclLine, int DeclColumn)
{
	...
	if (FoundEntry == NULL)
	{
		FoundEntry = VariableAlloc(pc, NULL, sizeof(struct TableEntry), Tbl->OnHeap);
		FoundEntry->Next = Tbl->HashTable[AddAt];
		Tbl->HashTable[AddAt] = FoundEntry;
	}

	FoundEntry->DeclFileName = DeclFileName;
	FoundEntry->DeclLine = DeclLine;
	FoundEntry->DeclColumn = DeclColumn;
	FoundEntry->p.v.Key = Key;
	FoundEntry->p.v.Val = Val;
